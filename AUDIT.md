...
> Dernière mise à jour : 20/08/2026 (v4.10.12, tranche 5 M5 — rituel du héron extrait).
...
| M5 | **God files** : `main.js` 3 392 lignes / 142 fonctions / 46 imports ; `render.js` `makeRenderer` 2 216 lignes | `src/main.js`, `src/render.js` | 🔄 **Chantier lancé** — **tranche 1 ✅ v4.10.7** (« La Crue » → `crue-controller.js`) · **tranche 2 ✅ v4.10.9** (« Dojo de parade » → `dojo-controller.js`) · **tranche 3 ✅ v4.10.10** (« Carte photo / partage » → `share-controller.js`) · **tranche 4 ✅ v4.10.11** (« Série de jours » → `streak-controller.js`) · **tranche 5 ✅ v4.10.12** (« Rituel du héron » → `heron-controller.js`, main.js **3 182 lignes**, 521 tests verts) ; reste : Trésors, Slots, Marché, Monde, Combat, Boot |
