...
> Dernière mise à jour : 25/08/2026 (v4.10.28, tranche 19 M5 — glue de progression extraite, main.js 1 383 lignes).
...
| | — | Bump v4.10.25 |
| **v4.10.26** | `—` | **Découpage de main.js — tranche 17** : « Cycle de vie complet » (opt-in, v4.2) extrait dans `src/lifecycle-controller.js` : `checkLifecycle` (promotion en aînée entre ELDER_AT et LIFE_MAX, puis grand départ paisible de vieillesse ou après trop longue absence au héron) + `passAway` (carte d'adieu + œuf de la génération suivante). `startNew` **reste dans main.js** (il RERELIE `s`/`mg` au jeu, comme l'import de save). `setupLifecycle` injecte getState/getRecords/persist/startNew ; appelé par `tick()` (démarre après tous les setups). **+1 test smoke** « aînée via tick » (exerce le câblage sans déclencher le setTimeout du grand départ). −32 lignes (1 542 → **1 511**). Ajouté au PRECACHE. 524 tests verts. |
| | — | Bump v4.10.26 |
| **v4.10.27** | `—` | **Découpage de main.js — tranche 18** : « Lieux » (berge ⇄ tanière ⇄ monde) extraits dans `src/lieux-controller.js` : `denAvailable` (tanière accessible : loutre là, dispo, hors mini-jeu — prédicat injecté au Monde et à la Crue), `updatePlaceBtn` (visibilité des boutons de lieu + classes CSS `in-den`/`in-world`/`in-game`/`in-dive`), `togglePlace`. `setupLieux` injecte getState/getMinigame/diving/persist/hintDone et est **hissé avant** l'appel `updatePlaceBtn()` du boot (motif A de l'audit régression). −34 lignes (1 511 → **1 477**). Ajouté au PRECACHE. 524 tests verts (dont boot-resume qui exerce updatePlaceBtn au boot) + vérif navigateur (bascule tanière OK, zéro erreur). |
| | — | Bump v4.10.27 |
| **v4.10.28** | `—` | **Découpage de main.js — tranche 19** : « Progression » (glue partagée) extraite dans `src/progress-controller.js` : `gainXp` (XP + montées de niveau : célébration, trésors de palier, gemmes, déblocages), `quest` (quêtes du jour), `varietyBonus` (1re activité variée du jour), `careBond` (lien avec la loutre), + helpers `questCtx`/`featuresOpenedBetween`. `checkUnlocks` et ses repères `prevHats`/`prevFurs` **restent dans main.js** (touchés par l'import de save et l'achat cosmétique) et sont passés à progress en hook. `setupProgress` injecté tôt dans le boot (la progression est appelée dès `checkStreak`). Les 8 contrôleurs gardent leurs hooks `gainXp`/`quest`/… qui pointent désormais vers les fonctions importées (pas de churn côté contrôleurs). −94 lignes (1 477 → **1 383**). Ajouté au PRECACHE. 524 tests verts + vérif navigateur (feed→XP/lien/quête, passage niv.1→2 : gems +2 + checkUnlocks OK, zéro erreur). |
| | — | Bump v4.10.28 |
